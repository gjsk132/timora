...
로그인할 때마다 Timora가 자동으로 켜지게 하려면:

1.  → **시스템 설정 → 일반 → 로그인 항목** 을 엽니다.
2. **로그인 시 열기** 아래의 **+** 버튼을 누릅니다.
3. **`Timora.app`** 을 (응용 프로그램 등 보관해둔 위치에서) 선택하고 **열기** 를 누릅니다.

이제 로그인할 때마다 Timora가 알아서 실행돼 메뉴바에 상주합니다.
해제하려면 같은 목록에서 Timora를 선택하고 **−** 를 누르면 됩니다.
